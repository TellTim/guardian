...
public final class AppBootUp {
    ...
    private final List<AppBootUpTask> startTaskNodes;
    private final Map<Class<? extends AppBootUpTask>, AppBootUpTask> taskMap;
    ...
    private AppBootUp(Builder builder) {
        ...
        int mainTaskCount = 0;
        for (AppBootUpTask task : builder.tasks) {
            ...
            List<Class<? extends AppBootUpTask>> dependencies = task.dependencies();
            if (dependencies != null && !dependencies.isEmpty()) {
                for (Class<? extends AppBootUpTask> clazz : dependencies) {
                    AppBootUpTask depTask = taskMap.get(clazz);
                    if (depTask != null) {
    ...
    public AppBootUp start() {
        ...
        for (AppBootUpTask task : startTaskNodes) {
            task.start();
        }
    ...
    private void onProjectFinish() {
        ...
        for (AppBootUpTask task : taskMap.values()) {
            if (!task.isFinished()) {
                if (config.isStrictMode) {
    ...
    public static class Builder {
        ...
        private final Map<Class<? extends AppBootUpTask>, AppBootUpTask> taskMap = new HashMap<>();
        private Config config;
        private List<AppBootUpTask> tasks;
        private Logger logger;

        public AppBootUp create() {
            if (config == null) {
                config = new Config();
            }
            if (logger == null) {
        ...
        public Builder add(AppBootUpTask task) {
    ...
    private class TaskStateListener implements TaskListener {
        ...
        @Override
        public void onWaitRunning(AppBootUpTask task) {

        }

        @Override
        public void onStart(AppBootUpTask task) {
        ...
        @Override
        public void onFinish(AppBootUpTask task, long dw, long df) {
            for (TaskListener taskListener : taskListeners) {
